...
export default function Page({}: Props) {
  ...
  const [selectedMeme, setSelectedMeme] = useState<LeaderboardMeme | null>(
    null
  );
  ...
  // Helper function to get live memes (memes from last 24 hours)
  const getLiveMemes = () => {
    const now = new Date();
  ...
  const getFilteredMemes = () => {
    if (activeTab === "live") {
      ...
      // For 'all' tab, exclude memes that are in the live section
      const liveMemes = getLiveMemes();
      const liveMemeIds = new Set(liveMemes.map((meme) => meme.vote_to._id));

      return allMemes.filter((meme) => !liveMemeIds.has(meme.vote_to._id));
    }
  };
    ...
    <div className="flex flex-col max-w-7xl mx-auto px-8">
          ...
          <div key={index} className="flex flex-col">
            ...
            <div className="flex justify-between items-center mb-2">
              <div
                ...
                onClick={() =>
                  router.push(`/home/profiles/${item.vote_to.created_by._id}`)
                }
              >
                <img
                  src={item.vote_to.created_by.profile_pic}
              ...
              <div className="hidden md:block ml-3 place-content-end space-y-6">
                ...
                >
                  <div className="flex flex-col items-center space-y-1">
                    <div className="w-[30px] h-[30px] flex items-center justify-center">
                      <FaRegBookmark
                        className="w-[30px] h-[30px] cursor-pointer hover:text-[#29e0ca] transition-colors"
                        onClick={() => handleBookmarkClick(item.vote_to._id)}
                      />
                    </div>
                    {activeTab === "all" && (
                      <p className="text-[#1783fb] font-bold text-lg">
                  ...
                  <div className="flex flex-col items-center space-y-1">
                    <div className="w-[30px] h-[30px] flex items-center justify-center">
                      <FaRegShareFromSquare
                        className="w-[30px] h-[30px] cursor-pointer hover:text-[#29e0ca] transition-colors"
                        onClick={() =>
                          handleShare(item.vote_to._id, item.vote_to.image_url)
                        }
                      />
                    </div>
                    {/* Share count can be added here if needed for "all" tab */}
                  </div>
                </Tooltip>
              </div>
            </div>
            ...
            <div className="flex justify-between items-center mt-1">
              <p
                ...
                onClick={() => handleOpenMeme(item, index)}
              >
                {item.vote_to.name.length > 30
                  ? item.vote_to.name.slice(0, 30) + "..."
                  : item.vote_to.name}
              </p>
              ...
              <div className="md:hidden flex items-center gap-4">
                ...
                <div className="flex flex-col items-center">
                  ...
                  {activeTab === "all" && (
                    <p className="text-center text-[#1783fb] font-bold text-sm mt-1">
                      {item.vote_to.vote_count}
                    </p>
                  )}
                </div>
                ...
                <div className="flex flex-col items-center">
                  <div className="w-[35px] h-[35px] flex items-center justify-center">
                    <FaRegBookmark
                      className="w-[25px] h-[25px] cursor-pointer hover:text-[#29e0ca] transition-colors"
                      onClick={() => handleBookmarkClick(item.vote_to._id)}
                    />
                  </div>
                  {activeTab === "all" && (
                    <p className="text-[#1783fb] font-bold text-sm mt-1">
                      {item.vote_to.bookmarks?.length || 0}
                    </p>
                  )}
                </div>
      ...
      {isMemeDetailOpen && selectedMeme && (
        <MemeDetail
          ...
          onNext={
            selectedMemeIndex < filteredMemes.length - 1
              ? handleNext
              : undefined
          }
