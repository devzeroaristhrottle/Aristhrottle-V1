...
const MemeCarousel: React.FC<MemeCarouselProps> = ({
  ...
  const [failedGoogleUrls, setFailedGoogleUrls] = useState<Set<string>>(
    new Set()
  )
  ...
  const validateGoogleCloudUrls = async (
    memes: MemeData[]
  ): Promise<MemeData[]> => {
    ...
    const validGoogleCloudMemes = validationResults.filter(
      (meme): meme is MemeData => meme !== null
    )
  ...
  const fetchLeaderboardMemes = async () => {
    try {
      ...
      const allTimeFiltered = allTimeMemes.filter(
        (allTimeMeme: MemeData) =>
          !dailyMemes.some(
            (dailyMeme: MemeData) => dailyMeme._id === allTimeMeme._id
          )
      )
      ...
      console.log(
        `Filtered ${
          combinedMemes.length - filteredMemes.length
        } memes with invalid URLs`
      )
  ...
  const handleImageError = (meme: MemeData) => {
    console.log(
      `Image failed to load for meme: ${meme.name} (${meme._id}) - URL: ${meme.image_url}`
    )

    if (isCloudinaryUrl(meme.image_url)) {
      console.warn(
        `Cloudinary URL detected in error handler: ${meme.image_url}`
      )
    }
      ...
      >
        <div className='flex flex-col items-center gap-4'>
          ...
          <p className='text-gray-500 text-sm'>
            All memes may have been filtered out
          </p>
        </div>
      </div>
    )
    ...
    <div className='p-2'>
      <div className='max-w-8xl mx-auto'>
        ...
        >
          ...
          <div
            className={`relative ${
              isMobile ? 'aspect-square' : 'h-96 md:h-70 md:w-70 lg:h-96'
            } p-2`}
          >
            {/* Cards container */}
            <div className='flex justify-center items-center h-full'>
              {isMobile ? (
                ...
                <div className='flex gap-6 h-full w-full justify-center max-w-7xl'>
                  ...
                  {!isMobile &&
                    getCurrentMemes().length < 3 &&
                    getCurrentMemes().length > 0 &&
                    Array.from({ length: 3 - getCurrentMemes().length }).map(
                      (_, emptyIndex) => (
                        <div
                          key={`empty-${emptyIndex}`}
                          className='flex-1 max-w-sm min-w-0 h-full opacity-30'
                        >
